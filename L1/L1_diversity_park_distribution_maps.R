...
species_raster_TD_mam <- raster("C:/Users/bgers/Desktop/MSU/Zarnetske_Lab/Data/Chapter_3/richness/Spec_rich_mam_TA.tif")
...
mam_td_density <- ggplot() + 
  ...
  scale_fill_manual(name = "TD Comparison", 
                    ...
                    labels = c("TD in parks","TD mams in all area")) +
...
# Load the raster of species richness
species_raster_FD_mam <- raster("C:/Users/bgers/Desktop/MSU/Zarnetske_Lab/Data/Chapter_3/richness/FD_mams_foraging_TA.tif")
...
mam_fd_density <- ggplot() + 
  ...
  scale_fill_manual(name = "FD Comparison", 
                    values = c("darkblue", "lightblue"), 
                    labels = c("FD in parks","FD mams in all area")) + 
  theme( panel.background = element_rect(fill = "white", color = "black", size = 1),
         plot.background = element_rect(fill = "white"),
...
# Add mean lines
raster_points_full_TD <- as.data.frame(rasterToPoints(species_raster_TD))
...
bird_td_density <- ggplot() + 
  ...
  scale_fill_manual(name = "TD Comparison", 
                    values = c("darkblue", "lightblue"), 
                    labels = c("TD in parks","TD birds in all area")) +
  theme( panel.background = element_rect(fill = "white", color = "black", size = 1),
         plot.background = element_rect(fill = "white"),
         legend.position = c(.95, .95),
         legend.justification = c("right", "top"),
...
bird_fd_density <- ggplot() + 
  ...
  scale_fill_manual(name = "FD Comparison", 
                    values = c("darkblue", "lightblue"), 
                    labels = c("FD in parks","FD Birds in all area")) + 
  theme( panel.background = element_rect(fill = "white", color = "black", size = 1),
         plot.background = element_rect(fill = "white"),
         legend.position = c(.45, .95),
         legend.justification = c("right", "top"),
...
#FD birds
# Load the raster of species richness
species_raster_bird_fd <- raster("C:/Users/bgers/Desktop/MSU/Zarnetske_Lab/Data/Chapter_3/richness/FD_birds_foraging_TA.tif")
# Extract the values of species richness at the centroid locations
...
test <-richness_map_mam_TD + circle_map_mam_TD + mam_td_density + richness_map_birds_TD +circle_map_birds_TD + bird_td_density + layout
