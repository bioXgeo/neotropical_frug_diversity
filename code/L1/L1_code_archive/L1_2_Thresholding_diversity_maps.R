...
for (percentage in upper_percentages) {
  ...
  panel_fd_joint <- ggplot() +
    ...
    scale_fill_manual(
      values = c("1" = "skyblue","0" = "cyan4"),
      labels = c("All FD areas", "FD above threshold"),
      guide = guide_legend(override.aes = list(shape = c(0, 1))),
      name = "Functional Diversity"
    ) + labs(title = paste("Frugivores - Top ", percentage, "% - Functional Diversity")) + ylab("Latitude") + xlab("Longitude") + scalebar(x.min = -83.5, x.max = -76.5, y.min = -24, y.max = -22.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(1.8, 0), legend.position = c(.50, .20), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 14, hjust = 0.5),  axis.text = element_text(size = 14),    axis.title = element_text(size=16),                                                legend.text = element_text(size = 14 ),  legend.title = element_text(size = 14)) + ylim(-25,12) +xlab("Longitude")+ ylab("Latitude")
  ...
  panel_sr_joint <- ggplot() +
    ...
    scale_fill_manual(
      ...
      guide = guide_legend(override.aes = list(shape = c(0, 1))),
      name = "Taxonomic Diversity"
    ) + labs(title = paste("Top ", percentage, "% - Taxonomic Diversity")) + ylab("Latitude") + xlab("Longitude") + scalebar(x.min = -83.5, x.max = -76.5, y.min = -27, y.max = -25.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(.9, 0), legend.position = c(.50, .20), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 14, hjust = 0.5),  axis.text = element_text(size = 14),    axis.title = element_text(size=16),                                                legend.text = element_text(size = 14 ),  legend.title = element_text(size = 14)) + ylim(-28,12) +xlab("Longitude")+ ylab("Latitude")
  ...
  panel_diversity_joint <-  ggplot() +
    ...
    ) + labs(title = paste("Top ", percentage, "% - Taxonomic and Functional Diversity")) + ylab("Latitude") + xlab("Longitude") + scalebar(x.min = -83.5, x.max = -76.5, y.min = -27, y.max = -25.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(.9, 0), legend.position = c(.50, .20), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 14, hjust = 0.5),  axis.text = element_text(size = 14),    axis.title = element_text(size=16),                                                legend.text = element_text(size = 14 ),  legend.title = element_text(size = 14)) + ylim(-28,12) +xlab("Longitude")+ ylab("Latitude")
  # Add the panels to the list
...
for (percentage in upper_percentages) {
  ...
  panel_fd_joint <- ggplot() +
    ...
    scale_fill_manual(
      ...
      guide = guide_legend(override.aes = list(shape = c(0,1, 2))),
      name = "Functional Diversity"
    ) + labs(title = paste("Frugivores - Top ", percentage, "% - Functional Diversity")) + ylab("Latitude") + xlab("Longitude") + 
    scalebar(x.min = -83.5, x.max = -76.5, y.min = -27, y.max = -25.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(1.2, 0), legend.position = c(.50, .20), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 20, hjust = 0.5),  axis.text = element_text(size = 20),    axis.title = element_text(size=20), legend.text = element_text(size = 14 ),  legend.title = element_text(size = 14)) + ylim(-28,12) +xlab("Longitude")+ ylab("Latitude")
  ...
  panel_sr_joint <- ggplot() +
    ...
    scale_fill_manual(
      ...
      guide = guide_legend(override.aes = list(shape = c(0, 1, 2))),
      name = "Taxonomic Diversity"
    ) + labs(title = paste("Top ", percentage, "% - Taxonomic Diversity")) + ylab("Latitude") + xlab("Longitude") + 
    scalebar(x.min = -83.5, x.max = -76.5, y.min = -27, y.max = -25.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(1.3, 0), legend.position = c(.50, .20), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 20, hjust = 0.5),  axis.text = element_text(size = 20),    axis.title = element_text(size=20), legend.text = element_text(size = 14 ),  legend.title = element_text(size = 14)) + ylim(-28,12) +xlab("Longitude")+ ylab("Latitude")
  ...
  panel_diversity_joint <-  ggplot() +
    ...
    scale_fill_manual(
      ...
      guide = guide_legend(override.aes = list(shape = c(0, 1,2))),
      name = "Frugivore Diversity"
    ) + labs(title = paste("Top ", percentage, "% - Taxonomic and Functional Diversity")) + ylab("Latitude") + xlab("Longitude") + 
...
fixed_joint_10 <- raster("C:/Users/bgers/Desktop/MSU/Zarnetske_Lab/Data/Chapter_3/richness/richness_rasters_binary/joint_rasters/diversity_joint_sum_10.tif")
...
panel_diversity_joint_fixed <-  ggplot() +
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(override.aes = list(shape = c(0, 1))),
    name = "Frugivore Diversity"
  ) + labs(title = paste("Top ", upper_percentages[1], "% - Taxonomic and Functional Diversity")) + ylab("Latitude") + xlab("Longitude") + 
  scalebar(x.min = -83.5, x.max = -76.5, y.min = -24, y.max = -22.5, dist=400, st.dist=.3, st.size=3.5, height=.5, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop,  location="bottomleft", scale=.065, symbol=1) +   theme(panel.background = element_rect(fill = "aliceblue", color="black"), legend.key.size = unit(1, 'cm'), legend.key.width = unit(1.2, "cm"), legend.justification = c(1.3, 0), legend.position = c(.80, .15), legend.box.background = element_rect(fill = "white"),    plot.title = element_text(size = 24, hjust = 0.5),  axis.text = element_text(size = 20),    axis.title = element_text(size=24),                                                legend.text = element_text(size = 20 ),  legend.title = element_text(size = 20)) + ylim(-25,12) +xlab("Longitude")+ ylab("Latitude")
