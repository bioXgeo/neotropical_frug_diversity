...
bird_traits_df_subset <- bird_traits_df %>% filter(X %in% colnames)


# Turn trait dataframe into a matrix
class(bird_traits_df)
...
bird_traits_cat <- as.data.frame(cbind(trait_name, trait_type))
...
#For birds the mad is lowest 4 dimensions. We can therefore go forward with 4 dimensions as this can decrease
...
writeRaster(fd_raster_bird$fdis_bird, "FD_birds_TA.tif", format="GTiff",overwrite=T)
