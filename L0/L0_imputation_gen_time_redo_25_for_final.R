...
df_real <- read.csv("C:/Users/bgers/Desktop/MSU/Zarnetske_Lab/Data/Chapter_3/tropical_subsets/frugivoria_TA_mammal_subset.csv")
...
for (method in methods) {
  ...
  for (i in 1:n_iterations) {
    ...
    standard_deviation <- sd(abs(differences), na.rm = TRUE)
  ...
  ####### Calculate the SD of means to get the bounds of SE
  sd_se[which(methods == method)] <- sd(method_average_differences)
  
  #### CART avg differences = 1.20 +- 1.87 
  
}

# Compare the standard deviations, mean standard deviations, and average differences across imputation methods
method_names <- c("Mean Matching", "Random Forest", "CART") ## did not look at the absolute values of differences so some canceled each other out. CART still performs the best
for (i in 1:length(standard_deviations)) {
  method_sd <- standard_deviations[[i]]
  method_diff <- average_differences[[i]]
  
  mean_sd <- mean_standard_deviations[i]
  sd_sd <- sd(method_sd)
  mean_diff <- average_differences[i]
  sd_mean_diff <- sd(method_diff)

  #### For final run through, need to make sure this value is the average difference for each method and the standard deviation of those differences across method
  ...
  cat(paste0(method_name, " - SD Average Difference: ", sd_mean_diff, "/n/n"))
}
